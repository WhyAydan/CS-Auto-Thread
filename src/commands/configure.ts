...
export const command: atCommand = {
	name: "configure",
	shortHelpDescription: "Modify the configuration of AT",

	async getSlashCommandBuilder() {
		return new SlashCommandBuilder()
			.setName("configure")
			.setDescription("Modify the configuration of AT")
			.addSubcommand(subcommand => {
				return subcommand
					.setName("message")
					.setDescription("Modify the content of a message that AT replies with when a certain action happens")
			...
			.addSubcommand(subcommand => {
				return subcommand
					.setName("default")
					.setDescription("Reset the server's custom AT configuration to the default");
			})
			.addSubcommand(subcommand => {
				return subcommand
	...
	async execute(interaction: CommandInteraction): Promise<void> {
		...
		if (interaction.options.getSubcommand() === "default") {
			...
			return interactionReply(interaction, success
				? "Successfully reset the AT configuration to the default."
