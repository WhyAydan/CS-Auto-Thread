...
async function getAllCommandsEmbed(): Promise<MessageEmbed> {
	const embed = new MessageEmbed().setTitle("🪡  AT Commands"); // :sewing_AT:
...
async function getCommandOptionString(cmd: atCommand): Promise<string> {
...
async function getCommandOptions(cmd: atCommand): Promise<APIApplicationCommandOption[] | undefined> {
	const commandInfo = await cmd.getSlashCommandBuilder();
