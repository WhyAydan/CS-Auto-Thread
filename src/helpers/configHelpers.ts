...
import type { atConfig } from "../types/atConfig";
import { MessageKey } from "./messageHelpers";

const CONFIGS_PATH = pathResolve(__dirname, "../../", process.env.CONFIGS_PATH || "configs");
const guildConfigsCache = new Map<string, atConfig>();

export function getConfig(guildId = ""): atConfig {
	const guildConfig = guildConfigsCache.get(guildId) ?? readConfigFromFile(guildId);

	const defaultConfigCopy = JSON.parse(JSON.stringify(defaultConfig)) as atConfig;
	if (guildConfig) {
...
function readConfigFromFile(guildId: string): atConfig | undefined {
	const path = getGuildConfigPath(guildId);
...
function setConfig(guild: Guild | null | undefined, config: atConfig): boolean {
